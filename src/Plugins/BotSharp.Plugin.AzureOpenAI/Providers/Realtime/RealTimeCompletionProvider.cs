...
using BotSharp.Plugin.AzureOpenAI.Models.Realtime;
using OpenAI.Chat;

namespace BotSharp.Plugin.AzureOpenAI.Providers.Realtime;

/// <summary>
/// Azure OpenAI Realtime API provider based on OpenAI Realtime implementation
/// Reference: https://docs.microsoft.com/en-us/azure/cognitive-services/openai/realtime-audio
/// </summary>
public class RealTimeCompletionProvider : IRealTimeCompletion
{
    ...
    private string _model = "gpt-4o-realtime-preview";
        ...
        Func<Task> onInterruptionDetected)
    {
        ...
        // Azure OpenAI Realtime endpoint format
        var endpoint = settings.Endpoint?.TrimEnd('/') ?? "https://your-resource.openai.azure.com";
        var apiVersion = "2024-10-01-preview"; // Azure OpenAI Realtime API version
        var deploymentName = _model;

        var uri = $"wss://{endpoint.Replace("https://", "")}/openai/realtime?api-version={apiVersion}&deployment={deploymentName}";
    ...
    private async Task ReceiveMessage(RealtimeModelSettings realtimeSettings)
    {
        ...
        await foreach (ChatSessionUpdate update in _session.ReceiveUpdatesAsync(CancellationToken.None))
        {
            ...
            {
                startTime = null;
                await TriggerModelInference("Respond to user immediately");
                continue;
            }
