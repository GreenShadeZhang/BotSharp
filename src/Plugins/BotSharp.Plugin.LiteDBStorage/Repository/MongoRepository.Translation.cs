...
public partial class MongoRepository
{
    public IEnumerable<TranslationMemoryOutput> GetTranslationMemories(IEnumerable<TranslationMemoryQuery> queries)
    {
        ...
        var memories = _dc.TranslationMemories.Find(x=>hashTexts.Contains(x.HashText)).ToList();
    ...
    public bool SaveTranslationMemories(IEnumerable<TranslationMemoryInput> inputs)
    {
        ...
        var memories = _dc.TranslationMemories.Find(x => hashTexts.Contains(x.HashText))?.ToList() ?? new List<TranslationMemoryDocument>();
        ...
        try
        {
            ...
            {
                _dc.TranslationMemories.InsertBulk(newMemories);
            }
            ...
            if (!updateMemories.IsNullOrEmpty())
            {
                ...
                {
                    _dc.TranslationMemories.Update(mem);
                    Thread.Sleep(50);
                }
            }
            return true;
