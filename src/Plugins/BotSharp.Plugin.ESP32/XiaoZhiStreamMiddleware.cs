...
/// <summary>
/// Reference to https://github.com/78/xiaozhi-esp32/blob/main/docs/websocket.md
/// </summary>
    ...
    private async Task HandleWebSocket(IServiceProvider services, string agentId, string conversationId, string sessionId, WebSocket webSocket)
    {
        ...
        do
        {
            ...
            if (result.MessageType == WebSocketMessageType.Text)
            {
                ...
                var response = JsonSerializer.Deserialize<JsonDocument>(receivedText);
                var type = response?.RootElement.GetProperty("type").GetString();

                _logger.LogInformation($"Received WebSocket message of type: {type}");
            }
