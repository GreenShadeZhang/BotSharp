...
public class RouteToAgentRoutingHandler : RoutingHandlerBase, IRoutingHandler
{
    ...
    public async Task<bool> HandleSseAsync(IRoutingService routing, FunctionCallFromLlm inst, RoleDialogModel message, Func<RoleDialogModel, Task> onResponseReceived)
    {
        var states = _services.GetRequiredService<IConversationStateService>();
        var goalAgent = states.GetState(StateConst.EXPECTED_GOAL_AGENT);
        if (!string.IsNullOrEmpty(goalAgent) && inst.OriginalAgent != goalAgent)
        ...
        {
            var ret = await routing.InvokeAgentSseAsync(agentId, _dialogs, onResponseReceived);
        }
