        ...
        Func<RoleDialogModel, Task> onStreamResponseReceived)
    {
        ...
        if (!stopCompletion)
        {
            ...
            if (agent.Type == AgentType.Routing)
            {
                ...
                response = await routing.InstructLoop(agent, message, dialogs, onStreamResponseReceived);
            }
            else
            {
                response = await routing.InstructDirect(agent, message, dialogs, onStreamResponseReceived);
            }
