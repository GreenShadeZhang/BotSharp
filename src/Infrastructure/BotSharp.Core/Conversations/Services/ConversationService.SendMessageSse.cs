...
{
    public async Task<bool> SendMessageSse(string agentId,
        ...
        Func<RoleDialogModel, Task> onMessageReceived)
    {
        ...
        if (!stopCompletion)
        {
            ...
            {
                response = await routing.InstructDirectSseAsync(agent, message, onMessageReceived);
            }

            routing.Context.ResetRecursiveCounter();
        }

        await HandleAssistantMessageSse(response, onMessageReceived);
        statistics.PrintStatistics();

        return true;
    }

    private async Task HandleAssistantMessageSse(RoleDialogModel response, Func<RoleDialogModel, Task> onResponseReceived)
    {
        var agentService = _services.GetRequiredService<IAgentService>();
